...
export class AuthInterceptor implements NestInterceptor {
    ...
    intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
        const authHeaderValue = context.switchToHttp().getRequest().headers['authorization'];
        const path = context.switchToHttp().getRequest().path;

        const pathRequireAuth = IGNORE_AUTH_ROUTES.indexOf(path) === -1;
        const invalidToken = authHeaderValue === undefined || !this.isTokenValid(authHeaderValue);
    ...
    private isTokenValid(authHeaderValue: string): boolean {
        try {
            ...
            const { exp } = this.jwtTokenService.verify(token, {secret: process.env.JWT_SECRET_KEY});
